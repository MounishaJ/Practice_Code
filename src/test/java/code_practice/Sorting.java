...
public class Sorting extends Base{
	...
	public List<String> List12() throws InterruptedException
	{
		...
		for(int i=0;i<=23;i++)
		{
		int count=driver.findElements(By.xpath("//td[@class=' capitalize'][1]")).size();
			for(int j=0;j<count;j++) 
			{
				Actual.add(driver.findElements(By.xpath("//td[@class=' capitalize'][1]")).get(j).getText());

			System.out.println(driver.findElements(By.xpath("//td[@class=' capitalize'][1]")).get(j).getText());
			}
				scrolldown();
				Thread.sleep(2000);
				p.pagearrow().click();
...
// Grab the table 
WebElement table = driver.findElement(By.id("customer_list")); 
...
/*		for(int i=0;i<=23;i++)
		{
		int count=p.listallrows().size();
		for(int j=0;j<count-1;j++)
		{
			System.out.println(p.data().get(i).getText());
		}
		//System.out.println(count);
		scroll(p.scrollingdown());
		p.pagearrow().click();
	*/	
